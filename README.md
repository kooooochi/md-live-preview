...
VSCode の拡張機能ビュー (`Cmd/Ctrl+Shift+X`) で `Markdown Sync Editor` を検索してインストール、または:

```bash
code --install-extension kooooochi.markdown-sync-editor
```
...
```bash
# 1. リポジトリを取得
git clone https://github.com/kooooochi/markdown-sync-editor.git
cd markdown-sync-editor
...
# 3. .vsix パッケージを生成
npx vsce package
# → markdown-sync-editor-0.1.0.vsix が生成される

# 4. VSCode にインストール
code --install-extension markdown-sync-editor-0.1.0.vsix
```
...
3. 生成された `markdown-sync-editor-0.1.0.vsix` を選択
...
2. コマンドパレット (`Cmd/Ctrl+Shift+P`) から `Markdown Sync: Open Synced Preview` を実行します
...
```markdown
![Screenshot](../media/screenshots/overview.png)
...
| 設定キー | 既定値 | 説明 |
|---------|--------|------|
| `markdownSyncEditor.mermaidTheme` | `default` | Mermaid のテーマ (`default` / `dark` / `forest` / `neutral`) |
| `markdownSyncEditor.enableEdit` | `true` | プレビュー上のダブルクリック編集を有効にする |
...
flowchart LR
  subgraph Extension["VSCode Extension"]
    command["markdownSyncEditor.open"]
    panel["PreviewPanel"]
...
Issue / PR 歓迎です。
[GitHub](https://github.com/kooooochi/markdown-sync-editor)
