...
3. 生成された `markdown-sync-studio-0.1.0.vsix` を選択
...
| 設定キー | 既定値 | 説明 |
|---------|--------|------|
| `markdownSyncStudio.mermaidTheme` | `default` | Mermaid のテーマ (`default` / `dark` / `forest` / `neutral`) |
| `markdownSyncStudio.enableEdit` | `true` | プレビュー上のダブルクリック編集を有効にする |
...
Issue / PR 歓迎です。
[GitHub](https://github.com/kooooochi/markdown-sync-studio)
